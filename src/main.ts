...
function homeView(): string {
  ...
  return `<div class="page home-page">
    <section class="hero" aria-labelledby="hero-title">
      ...
      <picture class="hero-art"><source media="(max-width: 700px)" srcset="/assets/notebook-hero-480.webp 480w, /assets/notebook-hero-720.webp 720w" sizes="100vw"><img src="/assets/notebook-hero-1280.webp" srcset="/assets/notebook-hero-720.webp 720w, /assets/notebook-hero-1280.webp 1280w" sizes="(max-width: 700px) 100vw, 48vw" width="1280" height="853" alt="A tactile paper notebook with an abstract mode slider, place pin, five comfort circles, and a pencil" fetchpriority="high" decoding="async"></picture>
    </section>
...
function bindEvents(): void {
  ...
  document.querySelector<HTMLFormElement>("[data-custom-place-form]")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget as HTMLFormElement);
    const place = String(data.get("place") ?? "").trim();
    if (!place) return;
    if (settings.customPlaces.some((item) => item.toLocaleLowerCase() === place.toLocaleLowerCase()) || DEFAULT_PLACES.some((item) => item.toLocaleLowerCase() === place.toLocaleLowerCase())) {
      showToast("That place tab already exists.");
      return;
    }
    settings.customPlaces = [...settings.customPlaces, place];
    await saveSettings(settings);
    render();
    showToast(`${place} added to your place tabs.`);
  });
  document.querySelectorAll<HTMLButtonElement>("[data-remove-place]").forEach((button) => button.addEventListener("click", async () => {
    const place = button.dataset.removePlace ?? "";
    settings.customPlaces = settings.customPlaces.filter((item) => item !== place);
    await saveSettings(settings);
    render();
    showToast(`${place} removed from your place tabs. Existing notes are unchanged.`);
  }));
